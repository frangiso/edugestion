...
function StudentsTab({ setSaving }) {
  ...
  async function removeStudent(id) {
    ...
    setSaving(false); setEditStudent(null); setStudentGrades([]);
  }
  ...
  async function saveTutor() {
    ...
    setSaving(false);
    setEditStudent(prev => ({ ...prev, tutorEmail: editTutor }));
    alert("Tutor actualizado");
  }

  async function selectStudent(s) {
    setEditStudent(s); setEditTutor(s.tutorEmail||"");
        ...
        <div className="card fade" style={{ padding:"24px", border:"2px solid #e0e7ff" }}>
          ...
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:"16px" }}>
            <div>
              ...
              <span className="badge" style={{ background:"#dbeafe", color:"#1e40af" }}>{editStudent.grade}</span>
            </div>
            <div style={{ display:"flex", gap:"12px", alignItems:"center" }}>
              <div style={{ textAlign:"center" }}>
                <div style={{ fontSize:"1.6rem", fontWeight:800, color: globalAvg==="–"?"#94a3b8":scoreColor(parseFloat(globalAvg)), fontFamily:"'Playfair Display',serif" }}>{globalAvg}</div>
                <div style={{ fontSize:"0.7rem", color:"#94a3b8" }}>Promedio general</div>
              </div>
          ...
          {loadingGrades ? <div style={{ color:"#94a3b8", fontSize:"0.85rem" }}>Cargando notas...</div> : (
            <>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:"10px", marginBottom:"16px" }}>
                {trimAvgs.map(({ t, avg: ta, count }) => (
                  <div key={t} style={{ textAlign:"center", padding:"10px", background:"#f8fafc", borderRadius:"10px", border:`2px solid ${ta==="–"?"#e2e8f0":scoreColor(parseFloat(ta))}` }}>
                    <div style={{ fontSize:"0.7rem", color:"#64748b", fontWeight:600, textTransform:"uppercase" }}>{trimNames[t-1]}</div>
                    <div style={{ fontSize:"1.5rem", fontWeight:800, color: ta==="–"?"#94a3b8":scoreColor(parseFloat(ta)), fontFamily:"'Playfair Display',serif" }}>{ta}</div>
                    <div style={{ fontSize:"0.68rem", color:"#94a3b8" }}>{count} eval.</div>
                  </div>
                ))}
              </div>
              ...
              {studentGrades.length > 0 && (() => {
                ...
                return (
                  <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
                    {Object.entries(subMap).map(([subject, sGrades]) => {
                      const sAvg = parseFloat(avg(sGrades.map(g=>g.score)));
                      return (
                        <div key={subject} style={{ padding:"10px 14px", background:"#f8fafc", borderRadius:"10px", border:"1px solid #e2e8f0", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                          <span style={{ fontWeight:600, color:"#1e293b", fontSize:"0.88rem" }}>{subject}</span>
                          <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                            {[1,2,3].map(t => {
            ...
            <div key={o.id} className="card" style={{ padding:"16px 20px", borderLeft:"4px solid #7c3aed" }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"8px" }}>
                <div>
                  ...
                  {o.subjects && o.subjects.length > 0 && <span style={{ fontSize:"0.78rem", color:"#94a3b8", marginLeft:"6px" }}>· {o.subjects.join(", ")}</span>}
                </div>
