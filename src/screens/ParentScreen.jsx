...
export default function ParentScreen({ user, profile, logout }) {
  ...
  async function loadData() {
    ...
    if (myChildren.length > 0) {
      ...
      const gMap = {}; const oMap = {}; const aMap = {};
      await Promise.all(myChildren.map(async c => {
        const [grades, obs, att] = await Promise.all([
          getGradesByStudent(c.id),
          getObservationsByStudent(c.id),
          getAttitudesByStudent(c.id),
        ]);
        gMap[c.id] = grades;
        oMap[c.id] = obs;
        aMap[c.id] = att;
  ...
  // Agrupa actitudinales por trimestre → por cada trimestre calcula promedio numérico de todos los valores
  ...
  // Por materia en cada trimestre — desglosa actitudinales
          ...
          <>
            ...
            <div className="card" style={{ padding:"24px 28px", marginBottom:"24px", display:"flex", justifyContent:"space-between", alignItems:"center", borderLeft:"5px solid #1e3a5f", flexWrap:"wrap", gap:"16px" }}>
              ...
              <div style={{ display:"flex", gap:"24px", textAlign:"center", flexWrap:"wrap" }}>
                ...
                {childAtt.length>0 && (() => {
                  const s = calcAttitudeSummary(0);
                  if (!s) return null;
                  return (
                    <div>
                      <div style={{ fontSize:"2rem", fontWeight:800, color:ATTITUDE_COLORS[s.value], fontFamily:"'Playfair Display',serif" }}>{s.value}</div>
