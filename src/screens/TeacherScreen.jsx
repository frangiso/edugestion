...
function StudentGradesTab({ user, subject }) {
  ...
  async function selectStudent(s) {
    ...
    const studentGrades = await getGradesByStudent(s.id);
  ...
  // Filtrar: notas de ESTE alumno en la materia activa, cargadas por este profesor
  const myGrades = allGrades.filter(
    g => g.studentId === selectedStudent?.id && g.subject === subject && g.teacherId === user.uid
  );
