...
import { searchStudents, getGradesByTeacher, createGrade, deleteGrade } from "../db";
...
export default function TeacherScreen({ user, profile, logout }) {
  ...
  async function loadGrades() {
    setLoading(true);
    const g = await getGradesByTeacher(user.uid);
    setGrades(g);
    setLoading(false);
  }
...
function AddGrade({ user, subject, grades, setGrades, setSaving }) {
  ...
  async function submit() {
    ...
    setSaving(true);
    const data = { ...form, score, teacherId: user.uid, subject, studentId: selectedStudent.id, studentName: selectedStudent.name };
...
function Ranking({ grades, subject }) {
  ...
  subjectGrades.forEach(g => {
    if (!studentMap[g.studentId]) studentMap[g.studentId] = { id:g.studentId, name:g.studentName||"–", grade:"", scores:[] };
  ...
  return (
    <div>
      ...
      {students.length === 0 ? (
        <div className="card" style={{ padding:"48px", textAlign:"center", color:"#94a3b8" }}>
          <p>No hay evaluaciones cargadas para esta materia aún.</p>
        </div>
      ) : (
