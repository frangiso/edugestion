...
export async function updateUserProfile(uid, data, role = null) {
  await updateDoc(doc(db, "users", uid), data);
  if (!role || role === "teacher") cache.teachers = null;
  if (!role || role === "parent") cache.parents = null;
}
...
export async function getAllGrades() {
  ...
  allGradesCache = snap.docs.map(d => ({ id: d.id, ...d.data() }));
  return allGradesCache;
}
...
// Usa el caché local para evitar una lectura de Firestore por cada guardado
export async function saveAttitude(data) {
  const teacherCache = cache.attitudesByTeacher[data.teacherId];
  const cached = teacherCache?.find(
    a => a.studentId === data.studentId &&
         a.subject === data.subject &&
         a.trimester === data.trimester
  );
  let id;
  if (cached) {
    id = cached.id;
